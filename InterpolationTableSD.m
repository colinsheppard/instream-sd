...
#import "InterpolationTableSD.h"


@implementation InterpolationTableSD

+ create: aZone
{

   InterpolationTableSD* interpolationTable = [super create: aZone];
   ...
   interpolationTable->previousX = -LARGEINT;
   interpolationTable->previousResult = -LARGEINT;
...
- (double) getValueFor: (double) anX
{
   ...
   if(useLogs == NO)
   {
	  // calculate result and store it in case next input is same
      previousResult = [self interpolateFor: anX];
	  return previousResult;
   }
   else  // perform logarithmic interpolation
   {
      ...
      // calculate result and store it in case next input is same
      previousResult =  exp([self interpolateFor: anX]);
	  return previousResult;
   }
