       ...
       if((varValueCount <= numberOfScenarios) && !ERROR)
       {
           if(         (strcmp(className,"FOO") != 0)
		    ...
		    //xprint(instanceName);
                    ...
                    if(instanceName == nil)
                    {
                        fprintf(stderr, "ERROR: ParameterManager >>>> initializeParameters >>>> instance is nil\n");
                        fflush(0);
                        exit(1);
                    }
                    */
    ...
    withInstanceName: (id <Symbol>) anInstanceName
{
  ...
  [modelIterator nextControlSetOnObject: aSwarmObject
   ...
   fprintf(stdout, "ExperBatchSwarm >>>> initializeModelFor >>>> END\n");
   fflush(0);
...
- setupModel
{
  ...
  fprintf(stdout, "ExperBatchSwarm >>>> setupModel >>>> after initializeModelFor\n");
  fflush(0);
  //
